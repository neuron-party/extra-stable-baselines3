...
def init_specific_all_levels_dict(levels_list, env_name):
    '''
    same as init_all_levels_dict but using a prespecified list of levels instead of a start and end
    '''
    init_state_bytes = {}
...
def create_demonstration_return_dict(d):
    '''
    For computing the value when moving boundaries back in continuous envs
    '''
    demonstration_returns = {}
    
    for key in d:
        returns = [d[key][i]['return'] for i in d[key].keys() if i != 'trajectory boundaries']
        average_return = np.mean(returns)
        demonstration_returns[key] = average_return
        
    return demonstration_returns



def postprocess_dict_continuous(d):
    d = d.copy()
    ...
    for level in d:
        trajectory_returns = [d[level][i]['return'] for i in d[level].keys() if i != 'trajectory boundaries']
        if sum(np.array(trajectory_returns) > 0) == 0:
            unplayable_levels.append(level)
        else:
            trajectory_popkeys = []
            for i in d[level]:
...
def init_all_level_trajectories_continuous(
    easy_levels,
    hard_levels, 
    pretrained_weights_path, 
    finetuned_weights_path, 
    ...
    for level in tqdm.tqdm(easy_levels):
        ...
        with torch.no_grad():
            for i in range(5): # number of trajectories to generate for a level
                sum_reward = tries = 0
                while sum_reward == 0 and tries < 3:
                    done = False
                    state = env.reset()
                    trajectory = [state]
                    trajectory_bytes = [env.env.get_state()]
