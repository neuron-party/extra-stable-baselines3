...
SelfResearchMethod4 = TypeVar("SelfResearchMethod4", bound="ResearchMethod4")


class ResearchMethod4(BaseAlgorithm):
    """
    boundary sampling, revised training average tracker, (not implemented yet) early stopping, dynamic p value
    
    the irm dictionary now has length 5 [0, boundary_1, boundary_2, boundary_3, dummy_boundary], for ease of coding
    """
    def __init__(
        self,
            ...
            for item in infos:
                if 'episode' in item.keys():
                    ...
                    if level not in self.init_research_method:
                        ...
                        # new_avg = old_avg*0.9 + latest_ret*0.1
            ...
            for idx, done in enumerate(dones):
                if done:
                    if np.random.random() <= self.p: # basically the same exact code, but we are only augmenting training with probability p (some parameter to set later)
                        ...
                        # [600 - end of list]
                        if len(self.init_research_method[hard_level]['trajectory boundaries']) == 5: # [0, b1, b2, b3, dummy b]
                            starting_boundary = np.random.randint(
                                low=self.init_research_method[hard_level]['trajectory boundaries'][-2],
                                high=len(self.init_research_method[hard_level][hard_level_trajectory]['trajectory bytes']) - 1
                            )
                        # play starting from 0 until its good
                        elif len(self.init_research_method[hard_level]['trajectory boundaries']) == 1: # > 2
                            starting_boundary = 0
                        # sample from in between 2 boundaries
                        else:
                            starting_boundary = np.random.randint(
                                low=self.init_research_method[hard_level]['trajectory boundaries'][-2],
                                high=self.init_research_method[hard_level]['trajectory boundaries'][-1]
                            )
