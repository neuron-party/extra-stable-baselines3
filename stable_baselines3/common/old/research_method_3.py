...
SelfResearchMethod3 = TypeVar("SelfResearchMethod3", bound="ResearchMethod3")


class ResearchMethod3(BaseAlgorithm):
    """
    single boundaries
    """
    def __init__(
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            # set environments
            for idx, done in enumerate(dones):
                # im looking at the nohup and its repeatedly saying manual reset ignored which suggests this loop is activating repeatedly but WHY??
                if done:
                    if np.random.random() <= self.p: # basically the same exact code, but we are only augmenting training with probability p (some parameter to set later)
                        ...
                        hard_level_trajectory = random.sample(list(self.init_research_method[hard_level].keys())[1:11], 1)[0] # fix this hardcoded indexing later
                        average_return = self.init_research_method[hard_level]['training average reward']

                        if average_return >= 5.0:
                            if len(self.init_research_method[hard_level]['trajectory boundaries']) > 1:
                        ...
                        starting_boundary = self.init_research_method[hard_level]['trajectory boundaries'][-1]
                        state_bytes = self.init_research_method[hard_level][hard_level_trajectory]['trajectory bytes'][starting_boundary]

                        current_state_bytes = env.venv.venv.env.env.env.env.get_state().copy()
                        current_state_bytes[idx] = state_bytes[0] # need to access the list
                        env.venv.venv.env.env.env.env.set_state(current_state_bytes.copy())
