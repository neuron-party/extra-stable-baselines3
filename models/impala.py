...
class SB_Impala_BC(nn.Module): # procgen and atari
    '''
    PPO Requirements for stable baselines 3 compatability:
        forward() needs to return action, value, log_prob, takes in a dictionary
        need an evaluate_actions() method that takes states and actions and returns the log prob, value, and entropy
    ...
    Had to fix some stuff for GAIL compatability
    '''
    def __init__(self, obs_space, num_outputs, lr):
        super().__init__()
    ...
    def predict(self, observation, state=None, episode_start=None, deterministic=False):
        '''
        need to implement this method for GAIL (imitation package)
        
        obs already extracted from rgb
        
        MAKE SURE TO ADD .DEVICE TO THE POLICY 
        '''
        with torch.no_grad():
            observation = torch.tensor(observation, device=self.device)
            x = self.preprocess(observation) # [64, 64, 64, 3]
            x = x.permute(0, 3, 1, 2)
            ...
            action = dist.sample()
        return action.detach().cpu().numpy(), state 
    
    
class SB_Impala_GAIL(nn.Module): # procgen and atari
    '''
    PPO Requirements for stable baselines 3 compatability:
        forward() needs to return action, value, log_prob, takes in a dictionary
        need an evaluate_actions() method that takes states and actions and returns the log prob, value, and entropy
