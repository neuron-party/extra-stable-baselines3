...
from stable_baselines3.ppo.ppo_batched_finetune import PPO_BatchedFinetune
...
def parse_args():
    ...
    parser.add_argument('--max-global-steps', type=int, default=4000000)
    ...
    parser.add_argument('--finetune-batch-size', type=int, default=10) # change batch size to 1 for regular fine tuning
    parser.add_argument('--trajectory-dict-path', type=str, default=None)
...
def main(args):
    ...
    for section, i in enumerate(range(0, len(hard_levels), args.finetune_batch_size)):
        
        finetune_levels = hard_levels[i:i+args.finetune_batch_size]
        # need a self.finetune_level_init_dict
        finetune_level_init_dict = init_specific_levels_dict(finetune_levels, args.env_name)
        
        print(f'Finetuning on: {finetune_levels}')
        ...
        trajectory_dict_path = args.trajectory_dict_path + '_' + str(section)

        agent = PPO_BatchedFinetune(
            policy=model,
            custom_policy=True,
            env=env,
            finetune_level_list=finetune_levels,
            trajectory_dict_path=trajectory_dict_path,
        ...
        agent.finetune_level_init_dict = finetune_level_init_dict.copy()
        ...
        }, args.save_path + '_' + str(section) + '_finetune.pth')
